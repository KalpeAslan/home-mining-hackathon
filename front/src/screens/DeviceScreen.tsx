...
export const DeviceScreen: FC<IProps> = ({}) => {
    ...
    const handleCheckbox = useCallback((workMode: TWorkMode) => {
        return () => {
            if (currentDevice) {
    ...
    return <DefaultLayout>
        <Text>
            Device Page {String(route.params.id)}
        </Text>
        <View style={{marginBottom: 50}}>
            <RNSpeedometer labels={speedometerLabels} size={200} value={50} maxValue={100}/>
        </View>
        {
            currentDevice && <View style={{flexDirection: 'row', justifyContent: 'center'}}>
                <Checkbox isActive={currentDevice.workMode === 'normal'} label={'Work Mode'} value={'normal'}
                          onPress={handleCheckbox('normal')}
                />
                <Checkbox isActive={currentDevice.workMode === 'high'} label={'Work Mode'} value={'high'}
